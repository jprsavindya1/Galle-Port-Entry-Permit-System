...
<div class="container" id="reason-list-container">
    <div class="company-dashboard-card">
        ...
        <form id="reason-search-form" method="GET" action="{{ route('admin.reasons.index') }}" class="row g-3 mb-3 align-items-end ajax-search-form" style="background:linear-gradient(135deg,#e3f2fd 0%,#f8fafc 100%);border-radius:0.75rem;padding:0.75rem;">
            ...
            <div class="col-md-2">
                <a href="{{ route('admin.reasons.index') }}" class="btn btn-secondary w-100 ajax-link" style="border-radius:0.5rem;font-weight:500;"><i class="bi bi-arrow-clockwise"></i> Reset</a>
            </div>
        </form>
        ...
        @if(method_exists($reasons, 'links'))
            <div class="mt-3">
                {{ $reasons->withPath(route('admin.reasons.index'))->withQueryString()->links() }}
            </div>
        @endif
    </div>
</div>
