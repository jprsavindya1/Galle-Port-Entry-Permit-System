...
@push('scripts')
    <script>
        ...
        window.cartData = @json(session('temporary_permit_cart', []));
    </script>
    ...
    <script>
        ...
        window.checkBlacklistStatus = function() {
            ...
            .then(data => {
                if (data.blacklisted) {
                    ...
                    isBlacklisted = false;
                    // Enable check availability button only if ID is valid and NIC is valid
                    if (isIdValid && isNicValid) {
                        const checkBtn = document.querySelector('button[onclick="checkAvailability()"]');
                        if (checkBtn) {
        ...
        window.handleIdNumberChange = function() {
            ...
            // Clear blacklist message when ID number changes
            const blacklistMsg = document.getElementById('blacklist_msg');
            if (blacklistMsg && !currentIdNumber) {
                blacklistMsg.textContent = '';
                blacklistMsg.style.color = '';
                isBlacklisted = false;
                
                // Re-enable check availability button if no ID number
                const checkBtn = document.querySelector('button[onclick="checkAvailability()"]');
                if (checkBtn && !currentIdNumber) {
                    checkBtn.disabled = false;
                    checkBtn.style.opacity = '1';
                    checkBtn.style.cursor = 'pointer';
                }
        ...
        window.syncIdType = function(idType) {
            ...
            if (checkbox.checked) {
                ...
                if (hasIdNumber && previousIdType !== '' && previousIdType !== idType) {
                    ...
                    // Clear NIC field and related messages
                    ...
                    // Clear blacklist messages and reset status
                    const blacklistMsg = document.getElementById('blacklist_msg');
                    if (blacklistMsg) {
                        blacklistMsg.textContent = '';
                        blacklistMsg.style.color = '';
        ...
        window.checkDuplicateInCart = function() {
            ...
            if (window.cartData && window.cartData.length > 0) {
                for (const entry of window.cartData) {
                    const cartIdNumber = (entry.id_number || '').toUpperCase();
            ...
            if (isDuplicate) {
                duplicateError.textContent = `⚠️ The person who this ${duplicateType} belongs to already has an entry in the cart. One person can only have one permit per submission.`;
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            window.validateNicNumber = function() {
                ...
                if (value === '' && nicInput.required) {
                    ...
                    isNicValid = false;
                    // Disable check availability button when NIC is required but empty
                    const checkBtn = document.querySelector('button[onclick="checkAvailability()"]');
                    if (checkBtn) {
                        checkBtn.disabled = true;
                        checkBtn.style.opacity = '0.6';
                ...
                if (!nicRegex.test(value)) {
                    ...
                    nicInput.classList.remove('is-invalid');
                    isNicValid = true;
                    // Re-enable check availability button only if ID is not blacklisted
                    if (!isBlacklisted) {
                        const checkBtn = document.querySelector('button[onclick="checkAvailability()"]');
                        if (checkBtn) {
                            checkBtn.disabled = false;
            ...
            function validateId() {
                ...
                if (!regex.test(value) && value !== "") {
                    ...
                    if (isIdValid && !isBlacklisted && isNicValid) {
                        const checkBtn = document.querySelector('button[onclick="checkAvailability()"]');
                        if (checkBtn) {
                            checkBtn.disabled = false;
                            checkBtn.style.opacity = '1';
            ...
            // Toggle NIC field visibility on page load
            toggleNicField();
        });
        ...
        window.checkAvailability = function() {
            ...
            const missingFields = [];
            if (!idType) missingFields.push('Identification Type');
        ...
        window.clearForm = function() {
            ...
            // Toggle NIC field visibility after resetting ID type
            toggleNicField();
            ...
            // Reset passport type radio buttons
            const passportTypeRadios = document.querySelectorAll('input[name="passport_type"]');
            passportTypeRadios.forEach(radio => {
                radio.checked = false;
                radio.removeAttribute('checked');
            });
        ...
        $(document).ready(function() {
            ...
            if (idTypeValue && idTypeValue !== '') {
                ...
                // If Passport, show passport type row and set default
                ...
                if (document.getElementById('doc_nic').checked) {
                    ...
                    idTypeDropdown.style.color = '#1976d2';
                    idTypeDropdown.style.fontWeight = '500';
                    // Show passport type row and set default
                    document.getElementById('passport_type_row').style.display = 'block';
                    if (!document.getElementById('local_passport').checked && !document.getElementById('foreigner_passport').checked) {
                        document.getElementById('local_passport').checked = true;
                    }
