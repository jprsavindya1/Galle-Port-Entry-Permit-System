...
<div class="container my-4">
    ...
    <select name="designation" id="designation" class="form-select" required>
            ...
            <tbody>
                @foreach(session('monthly_permit_cart') as $index => $permit)
                    <tr>
                        ...
                        <td>{{ $permit['reason'] }}</td>
                        <td><a href="{{ route('permit.monthly.editMonthlySessionEntry', $index) }}" class="btn btn-sm btn-warning">Edit</a></td>
                        <td>
                            <form method="POST" action="{{ route('permit.monthly.removeMonthlySessionEntry', $index) }}" style="display:inline;" onsubmit="return confirm('Are you sure you want to remove this entry?');">
                                @csrf
                                @method('DELETE')
        ...
        <form method="POST" action="{{ route('permit.monthly.submitAll') }}">
            @csrf
...
@push('scripts')
    ...
    $('#company_name').on('select2:open', function () {
        setTimeout(() => {
            document.querySelector('.select2-search__field').placeholder = 'Search company...';
        }, 10);
    });
   ...
   function setCompanyAddress() {
    ...
    // Initialize Select2 on company_name
    $(document).ready(function() {
        $('#company_name').select2({
            placeholder: "-- Select Company --",
            allowClear: true
